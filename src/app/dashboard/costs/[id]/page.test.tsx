...
jest.mock('@/components/pages/costs/cost-view', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`COST_VIEW_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}`,
		);
	},
}));

const AUTH_LOGIN = '/login';
const COSTS_LIST = '/dashboard/costs';
...
describe('CostViewPage server component', () => {
	...
	it('renders CostViewClient with session and id when session exists', async () => {
	...
	it('redirects to COSTS_LIST when id is invalid', async () => {
		const sessionValue: Session = { user: { pk: 20, email: 'viewer@site.com' } };
		mockAuth.mockResolvedValueOnce(sessionValue);
		...
		expect(mockRedirect).toHaveBeenCalledWith(COSTS_LIST);
	});
});
