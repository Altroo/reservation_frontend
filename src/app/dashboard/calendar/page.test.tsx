...
jest.mock('@/components/pages/reservations/calendar-client', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `CALENDAR_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('CalendarPage server component', () => {
	...
	it('renders CalendarClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 5, email: 'user@site.com' } };
