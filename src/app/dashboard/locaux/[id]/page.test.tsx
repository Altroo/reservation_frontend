...
jest.mock('@/components/pages/locaux/local-view', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`LOCAL_VIEW_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}`,
		);
	},
}));
...
describe('LocalViewPage server component', () => {
	...
	it('renders LocalViewClient with session and id when session exists', async () => {
	...
	it('redirects to LOCAUX_LIST when id is invalid', async () => {
		const sessionValue: Session = { user: { pk: 20, email: 'viewer@site.com' } };
		mockAuth.mockResolvedValueOnce(sessionValue);
