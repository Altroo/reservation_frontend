...
jest.mock('@/components/pages/locaux/locaux-dashboard', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `LOCAUX_DASHBOARD_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('LocauxDashboardPage server component', () => {
	...
	it('renders LocauxDashboardClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 8, email: 'dashboard@site.com' } };
		...
		expect(decoded).toContain('"email":"dashboard@site.com"');
