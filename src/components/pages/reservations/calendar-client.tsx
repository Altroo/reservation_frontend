...
import { PAYMENT_SOURCE_BG, MONTH_NAMES, DAY_ABBREVIATIONS, APARTMENT_COLORS } from '@/utils/rawData';
...
function buildDayMap(
	...
	for (const [aptName, apt] of Object.entries(apartments)) {
		...
		const aptColor = PAYMENT_SOURCE_BG[apt.reservations[0]?.payment_source] ?? APARTMENT_COLORS[colorIdx].replace('0.8)', '1)');
...
const CalendarContent: React.FC<CalendarContentProps> = ({ token }) => {
	...
	const { data: planning, isLoading, refetch } = useGetPlanningQuery(
		{ year, month },
		{ skip: !token },
	);
