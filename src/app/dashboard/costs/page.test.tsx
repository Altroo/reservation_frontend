...
jest.mock('@/components/pages/costs/costs-list', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `COSTS_LIST_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('CostsPage server component', () => {
	...
	it('renders CostsListClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 3, email: 'admin@costs.com' } };
		...
		expect(decoded).toContain('"email":"admin@costs.com"');
